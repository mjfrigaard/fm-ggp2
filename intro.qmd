...
The `ggplot2` syntax is comprised of layers:

1. Data: The **data layer** consists of a rectangular object (like a spreadsheet) with columns and rows
...
2. Mapping: The **mapping layer** assigns columns (variables) from the data to a visual property (i.e. graph ’**`aes`**’ thetic)
...
3. Statistics & Geometric objects: `geom_*()` functions include statistical transformations, shapes, and position adjustments for how to ‘draw’ the data on the graph
...
4. Position adjustments: With a finite number of objects & functions, we can combine `ggplot2`’s grammar and syntax to create an infinite number of graphs!
...
I've attempted to write code for each graph so that is can be read and understood *without* having to execute it.[^intro-1]
